...
In machine learning tasks decision trees perform well when the relationship between the input and the response is non-linear.
314
...
Therefore it follows the gradient bossted decision trees should perform well.

Rarely datasets and models in the wild are straightforward in the same way textbook examples are. It can be hard to understand exactly how a model is making its predictions. When I was first learning the boosting algorithm I came across a (post)[] outlining in a regression context how boosting gradually moves closer to the goal output. 

In this post I highlight the algorithm, write code to fit this model, apply it to a dataset where the output is a non-linear function of the input, and visualize the boosting process and associated decrease in the loss function. 
...
The boosting procedure is an iterative process, which lends itself nicely to visualization. In the model fitting code we saved the values of the loss function after every iteration, as well as the fitted values of the model. Here we'll construct a visualization of the process. 
...
As seen in the above output, as the data are exposed to more trees, the predictions become better. 
...
It is generally true that model fit metrics strictly improve on the training dataset as model complexity grows. This is not the case with validation datasets. There comes a point the model begins to learn the idiosyncracies of the training data rather than a more general relationship between input features and output of the data generating mechanism. The appropriate model to use in deployment would be the one that performs the best on the validation dataset, even if it is less complex and doesn't perform as well on the training dataset. 

Hyperparameter tuning and model selection is not the focus of this post, but several details in this example are relevant to its application. The hyperparameters here are the number of trees ($B$), the value of the learning rate ($\lambda$), and the maximum depth of each tree (1). In this example $\lambda$ and maximum tree depth remained fixed and we iterated our model fitting over $B$ different trees. A fuller treatment would involve fitting multiple models to the training data at varying levels of $\lambda$ and maximum tree depth and comparing model performance on the validation dataset at varying levels of number of trees fit (anywhere from $1$ to $B$).
